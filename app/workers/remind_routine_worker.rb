class RemindRoutineWorker
  ...
  def perform(device_type, device_token, message, title, id)
    if device_type == "ios"
      APNS.port = 2195
      APNS.pem  = pem
      APNS.pass = ''
      ...
      begin
        APNS.send_notification(device_token, :alert => { title: title, body: message, id: id} , :badge => 1, :sound => 'default')
      rescue Exception => ex
        return false
      end
      ...
      options = {data: {title: title, body: message, id: id}}
